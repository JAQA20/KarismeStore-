...
const CatalogKids = () => {
  ...
  const kidsProducts = allProducts.filter(
    (p) => p.gender === 'Niños' || p.gender === 'niños' || p.gender === 'Infantil'
  );
  ...
  const dbKidsCategoryNames = dbCategories
    .filter((c) => c.gender === 'Niños')
    .map((c) => c.name);

  // Baseline categories for Kids catalog to ensure menu categories NEVER disappear
  const defaultCategoryNames = dbKidsCategoryNames.length > 0 ? dbKidsCategoryNames : [
    'Pijamas',
    'Conjuntos',
    'Ropa de Estar',
    'Básicos',
  ...
  const allCategoryNames = Array.from(
    new Set([
      ...
      ...kidsProducts.map((p) => p.category).filter(Boolean),
    ])
  );
  ...
  const categories = [
    ...
    ...allCategoryNames.map((catName) => ({
      name: catName,
      count: kidsProducts.filter((p) => p.category === catName).length,
    })),
  ];

  const allAvailableSizes = sortSizes(
    Array.from(new Set(kidsProducts.flatMap((p) => p.availableSizes || [])))
  );
  const sizes = allAvailableSizes.length > 0 ? allAvailableSizes : ['2Y', '4Y', '6Y', '8Y', '10Y'];
  ...
  return (
    <main className="max-w-container-max mx-auto px-margin-desktop py-12 flex flex-col md:flex-row gap-12 font-sans">
      ...
      <aside className="w-full md:w-64 flex-shrink-0 space-y-10 sticky top-24 h-fit">
        ...
        {/* Reset Filters */}
        {(activeGender !== 'Todos' || activeCategory !== 'Todos' || activeColor || activeSize || maxPrice > 0) && (
          <button
        ...
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-outline-variant/20">
          <div>
            <h1 className="font-headline-lg text-headline-lg text-on-surface">
              Colección Infantil ({filteredProducts.length})
            </h1>
            <p className="font-body-md text-on-surface-variant text-sm mt-1">
              Pijamas y prendas suaves hipoalergénicas para niños y niñas.
            </p>
          </div>
        </header>
        ...
        {filteredProducts.length === 0 && (
          <div className="text-center py-16 bg-surface-container-low rounded-2xl border border-dashed border-outline-variant/40 space-y-3">
            ...
            <p className="text-sm font-bold text-on-surface-variant">
              No se encontraron productos en la colección infantil.
            </p>
            <button
              onClick={resetFilters}
