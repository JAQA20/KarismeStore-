...
const CatalogMen = () => {
  ...
  const menProducts = products.filter(p => p.gender === 'Hombre');
  ...
  const categories = [
    ...
    { name: 'Ropa de Estar', count: menProducts.filter(p => p.category === 'Ropa de Estar').length },
    { name: 'Básicos', count: menProducts.filter(p => p.category === 'Básicos').length },
    { name: 'Pijamas', count: menProducts.filter(p => p.category === 'Pijamas').length },
    { name: 'Ediciones Limitadas', count: menProducts.filter(p => p.category === 'Ediciones Limitadas').length }
  ];
    ...
    <main className="max-w-container-max mx-auto px-margin-desktop py-12 flex flex-col md:flex-row gap-12">
      ...
      <section className="flex-1">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 pb-6 border-b border-outline-variant/20">
          <div>
            <h1 className="font-headline-lg text-headline-lg mb-2">Colección Hombre</h1>
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="py-24 text-center space-y-6 bg-surface-container-low/40 rounded-2xl border border-outline-variant/20 p-12">
            ...
            <p className="font-body-md text-on-surface-variant max-w-md mx-auto">
              No se encontraron productos que coincidan con los filtros seleccionados para la colección hombre.
            </p>
            <button 
              onClick={resetFilters}
