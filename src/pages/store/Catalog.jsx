...
const Catalog = () => {
  ...
  const womenProducts = products.filter(p => p.gender === 'Mujer' || !p.gender);
  ...
  const categories = [
    ...
    { name: 'Lencería de Seda', count: womenProducts.filter(p => p.category === 'Lencería de Seda').length },
    { name: 'Ropa de Estar', count: womenProducts.filter(p => p.category === 'Ropa de Estar').length },
    { name: 'Básicos de Algodón', count: womenProducts.filter(p => p.category === 'Básicos de Algodón').length },
    { name: 'Ediciones Limitadas', count: womenProducts.filter(p => p.category === 'Ediciones Limitadas').length }
  ];
    ...
    <main className="max-w-container-max mx-auto px-margin-desktop py-12 flex flex-col md:flex-row gap-12">
      ...
      <section className="flex-1">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 pb-6 border-b border-outline-variant/20">
          <div>
            <h1 className="font-headline-lg text-headline-lg mb-2">Colección Mujer</h1>
            <p className="font-body-md text-body-md text-on-surface-variant/70">
              Mostrando <span className="font-bold text-on-surface">{filteredProducts.length}</span> resultados para <span className="font-bold text-on-surface">{activeCategory}</span>
        ...
        {filteredProducts.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-16">
            {filteredProducts.map((product) => (
              <div key={product.id} className="group product-card-hover flex flex-col items-center">
                ...
                <p className="font-label-sm text-label-sm text-on-surface-variant mb-2 uppercase">{product.colorName || product.color}</p>
          ...
          <div className="py-24 text-center space-y-6 bg-surface-container-low/40 rounded-2xl border border-outline-variant/20 p-12">
            ...
            <p className="font-body-md text-on-surface-variant max-w-md mx-auto">
              No se encontraron productos que coincidan con los filtros seleccionados para la colección mujer.
            </p>
            <button 
              onClick={resetFilters}
